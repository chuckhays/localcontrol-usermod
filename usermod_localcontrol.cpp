...
private:
  ...
  KY040Encoder rotary1{5, 6, 7,
                       [this](int delta)
                       { onEncoderRotate(EncoderId::Rotary1, delta); },
                       [this]()
                       { onEncoderClick(EncoderId::Rotary1); },
                       [this]()
                       { onEncoderLongPress(EncoderId::Rotary1); }};
  KY040Encoder rotary2{8, 18, 17,
                       [this](int delta)
                       { onEncoderRotate(EncoderId::Rotary2, delta); },
                       [this]()
                       { onEncoderClick(EncoderId::Rotary2); },
                       [this]()
                       { onEncoderLongPress(EncoderId::Rotary2); }};
  ...
  void drawIconArrow(int x, int y, uint16_t color, ArrowDir dir = ArrowDir::Right)
  {
    ...
    {
    case ArrowDir::Right:
      tft.fillTriangle(x, y + 10, x + 18, y + 1, x + 18, y + 18, color);
      break;
    case ArrowDir::Left:
      tft.fillTriangle(x + 19, y + 10, x + 1, y + 1, x + 1, y + 18, color);
      break;
    case ArrowDir::Up:
      tft.fillTriangle(x + 10, y, x + 1, y + 18, x + 18, y + 18, color);
      break;
    case ArrowDir::Down:
      tft.fillTriangle(x + 10, y + 19, x + 1, y + 1, x + 18, y + 1, color);
      break;
    }
  }
  ...
  {
    bool found = false;
    if (!requestJSONBufferLock(JSON_LOCK_PRESET_NAME))
      return false;
  ...
  void startMenuScan(bool preserveSelection)
  {
    menuScanRestoreId = (preserveSelection && listCursorIndex < (int)menuEntries.size())
                            ? menuEntries[listCursorIndex].id
                            : 0;
    menuEntries.clear();
    menuScanNextId = 1;
    menuScanActive = true;
  ...
  {
    if (e.isPlaylist)
      return currentPlaylist == (int16_t)e.id;
  ...
  {
    if (idx < listScrollOffset || idx >= listScrollOffset + MENU_VISIBLE)
      return;
  ...
  void drawPanelItem(int row, bool selected)
  {
    ...
    {
    case PanelItem::Brightness:
      tft.drawCentreString(String(bri), textCX, textCY, 2);
      break;
    case PanelItem::Speed:
      tft.drawCentreString(String(effectSpeed), textCX, textCY, 2);
      break;
    case PanelItem::Intensity:
      tft.drawCentreString(String(effectIntensity), textCX, textCY, 2);
      break;
    case PanelItem::Power:
      tft.drawCentreString(isOn ? "ON" : "OFF", textCX, textCY, 2);
      break;
    default:
      break;
    }
...
public:
  ...
  void loop() override
  {
    ...
    if (listInitialized && !menuScanActive)
    {
      ...
      if (activeId != lastActiveId)
      {
        ...
        for (size_t i = 0; i < menuEntries.size(); i++)
        {
          if (menuEntries[i].id == (uint16_t)lastActiveId)
            prevIdx = (int)i;
          if (menuEntries[i].id == (uint16_t)activeId)
            newIdx = (int)i;
        }
        lastActiveId = activeId;
        if (newIdx >= 0)
          listCursorIndex = newIdx;
  ...
  void appendConfigData(Print &settingsScript) override
  {
    ...
    settingsScript.print(F(":great',1,'<i>(this is a great config value)</i>');"));
    settingsScript.print(F("addInfo('"));
    settingsScript.print(FPSTR(_name));
    settingsScript.print(F(":testString',1,'enter any string you want');"));
    settingsScript.print(F("dd=addDropdown('"));
    settingsScript.print(FPSTR(_name));
    settingsScript.print(F("','testInt');"));
